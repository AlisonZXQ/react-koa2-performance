...
function Performance(option, fn) {
    try {
        ...
        var getResourceTime = (resource) => {
            let resourceTime = 0;
            resource.forEach(it => {
                resourceTime += Number(it.duration);
            })
            return resourceTime;
        }
            ...
            setTimeout(() => {
                ...
                if (type === 1) {
                    ...
                    result = Object.assign(result, {
                        ...
                        performance: {
                            ...conf.performance,
                            resourceTime: getResourceTime(conf.resourceList),
                        },
        ...
        var getLargeTime = () => {
            if (conf.page !== location.href) {
                ...
                if (conf.haveFetch && loadTime && fetchTime) {
                    console.log('页面级性能上报包括请求', "loadTime:" + loadTime + ",fetchTime:" + fetchTime);
                    reportData(1);
                } else if (!conf.haveFetch && loadTime) {
                    console.log('页面级性能上报', "loadTime:" + loadTime);
                    reportData(1);
                }
            } else {
                // 单页面内ajax上报
                if (conf.haveFetch && fetchTime) {
                    console.log('页面内单个请求上报', "fetchTime:" + fetchTime);
                    reportData(2);
                }
            }
        }
        ...
        var perforPage = () => {
            ...
            conf.performance = {
                ...
                //TCP建立时间
                tcpTime: timing.connectEnd - timing.connectStart || 0,
                // 白屏时间  
                whiteTime: timing.responseStart - timing.navigationStart || 0,
                //dom渲染完成时间
                domTime: timing.domContentLoadedEventEnd - timing.navigationStart || 0,
                //页面onload时间
                loadTime: timing.loadEventEnd - timing.navigationStart || 0,
                // 页面准备时间 
                readyTime: timing.fetchStart - timing.navigationStart || 0,
                // 页面重定向时间
                ...
                //页面解析dom耗时
                analysisDomTime: timing.domComplete - timing.domInteractive || 0,
            };
        };
