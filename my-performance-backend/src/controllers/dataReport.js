    ...
    async getPageResources(ctx) {
        ...
        try {
            ...
            if (!systemMsg || !systemMsg.length) {
                ctx.body = {
                    code: 402,
                    message: '!systemMsg || !systemMsg.length不存在'
                };
                return;
            };
            let systemItem = systemMsg[0]
            ...
            if (systemItem.isStatisiPages === 0) {
                let pageTimes = resourceDatas.performance || {}
                let datas = {
            ...
            resourceDatas.resourceList.forEach(async item => {
                let duration = 0;
                let table = ''
            ...
            if (systemItem.isStatisiResource !== 0) {
                ctx.body = {
                    code: 402,
                    message: 'isStatisiResource ！==0'
                };
                return;
            };

            datas.resourceDatas = JSON.stringify(resourceDatas.resourceList)
